...
using vehicle_core::ActualGear;
using vehicle_core::Availability;
using vehicle_core::Callback;
using vehicle_core::DecoderObservation;
using vehicle_core::FrontWiperPosition;
...
using vehicle_core::RawCanFrame;
using vehicle_core::Reading;
using vehicle_core::SelectorPosition;
using vehicle_core::TurnState;
...
// the facade while allowing later modules to be implemented independently.
struct DecoderInput {
  RawCanFrame frame{};
};
...
struct LightingUpdate {
  ...
  MonotonicTimestamp valid_until_us{0};
};
